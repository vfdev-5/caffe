...
void GPUMemory::Manager::update_dev_info(int device) {
  ...
  dev_info_[device].total_ = std::min<size_t>(props.totalGlobalMem, dev_info_[device].total_);
  dev_info_[device].free_ = std::min<size_t>(dev_info_[device].total_, dev_info_[device].free_);
